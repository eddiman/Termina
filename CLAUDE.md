...
- **`lib.rs`** — App setup, plugin registration, background threads (process monitor every 3s, health checker every 15s), window lifecycle, global shortcut (Cmd+Shift+T), orphaned process cleanup on startup
...
- `tauri_plugin_global_shortcut::GlobalShortcutExt` for shortcut registration
